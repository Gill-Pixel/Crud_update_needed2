...
const createPanel = (event) => {
  ...
  area = $(`.animals .${panel_class}`); // Move this line outside the forEach loop
  ...
  let createPara = $(`<span>Use the CREATE button below to register a new ${panel_class} for the shelter.</span>
  ...
  $(".displayLine button").on("click", (e) => {
    ...
    if (elementClass === "update") {
      const index = target.closest(".displayLine").index();
      updateHandler(index);
    } else {
      handler(target["0"]);
    }
...
const updateHandler = (index) => {
  ...
  // Clear existing content and show update input fields
  area.html(`
    <div class="info">
  ...
  $(".crud_buttons .save_update").on("click", function () {
    saveUpdate(index);
  });
};

// New function to save the update
const saveUpdate = (index) => {
  // Update the corresponding animal's data in the array
  animals[index] = {
  ...
  createPanel({ target: { id: panel_class } });

  // Scroll to the list of dogs with updated information
  $('html, body').animate({
...
const createHandler = () => {
  ...
  let createPara = $(`<input type="text" id="new_name" value='name' size="6"/> 
    is a 
